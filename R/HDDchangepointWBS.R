...
argmaxDelta <- function(data,FUN=delta0){
  nn <- nrow(data)
  d <- ncol(data)
...
test_statistic_wbs <- function(data,FUN=delta0,cp_estimate=NULL){
  nn <- nrow(data)
  d <- ncol(data)
  distance_matrix <- as.matrix(FUN(data))
  if(identical(FUN,dist))
...
test_significance_changepoint <- function(data,FUN=delta0,cp_estimate=NULL,test="asymptotic",npermut=200,siglevel=0.05){
  nn <- nrow(data)
  d <- ncol(data)
...
#' @param M             number of random draws for wild binary segmentation, default is 200
#' @return    the list of detected significant change points, including any NA's which should be removed then
#' @export

multiple_changepoint_detection_wbs <- function(data,FUN=dist,minsegment=10,M=100,test="asymptotic",npermut=200,siglevel=0.05){
  changepoint_all <- NA
  M <- M
  ...
  while(length_data>minsegment){
    ...
    if(is_changepoint==1 & changepoint>2 & changepoint<nn_split-2){
      ...
      changepoint_before <- multiple_changepoint_detection_wbs(data=data_before,FUN=FUN,minsegment=minsegment,test=test,npermut=npermut,siglevel=siglevel)
      changepoint_after <- multiple_changepoint_detection_wbs(data=data_after,FUN=FUN,minsegment=minsegment,test=test,npermut=npermut,siglevel=siglevel)+changepoint
